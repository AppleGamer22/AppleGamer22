...
for i in range(len(cipher_ascii)):
	...
	for ii in range(len(key)):
		rotated_key = key[len(known_cleartext) - ii % len(known_cleartext):len(known_cleartext)] + key[:len(known_cleartext) - ii % len(known_cleartext)]
		message_text = str(xor(cipher_ascii, rotated_key))[2:-1]
